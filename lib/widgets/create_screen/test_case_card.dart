...
class _TestCaseCardState extends State<TestCaseCard> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.result != null) {
      borderColor =
          widget.result!.passed ? const Color(0xFF4CAF50) : Colors.red;
    }
        ...
        child: Column(
          ...
          children: [
                ...
                children: [
                  Expanded(
                    ...
                    child: _MonoField(
                      ...
                      onChanged: (v) =>
                          widget.test.inputFile = v.isEmpty ? null : v,
                ...
                children: [
                  Expanded(
                    ...
                    child: _MonoField(
                      ...
                      onChanged: (v) =>
                          widget.test.outputFile = v.isEmpty ? null : v,
            ...
            Row(
              children: [
                Checkbox(
                  ...
                  onChanged: (v) =>
                      setState(() => widget.test.isHidden = v ?? false),
                ),
                GestureDetector(
                  onTap: () => setState(
                      () => widget.test.isHidden = !widget.test.isHidden),
                  child: Row(
                    children: [
                ...
                if (_inputMode == 'file')
                  _IoTag(
                      'IN → ${widget.test.inputFile ?? "file"}', Colors.blue),
                if (_outputMode == 'file') ...[
                  const SizedBox(width: 6),
                  _IoTag(
                      'OUT → ${widget.test.outputFile ?? "file"}', Colors.teal),
                ],
              ],
            ),
            if (widget.result?.error != null &&
                widget.result!.error!.isNotEmpty) ...[
              const Divider(),
              Container(
            ...
            if (widget.result != null &&
                ...
                widget.result!.actualOutput.isNotEmpty &&
                widget.result?.error == null) ...[
              const Divider(),
              Container(
            ...
            if (widget.result?.compileError != null &&
                widget.result!.compileError!.isNotEmpty) ...[
              const Divider(),
              Container(
                padding: const EdgeInsets.all(8),
                decoration: BoxDecoration(
...
// Define the data structure and initialize the Section Header.
...
// Define the data structure and initialize the Mode Toggle.
...
// Provide interface component for Mono Field.
...
// Define the data structure and initialize the IO Tag.
